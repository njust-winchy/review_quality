...
def main(args):
    ...
    # dataset
    train_data_file = os.path.join(args.data_dir, "claim_train.json")
    test_data_file = os.path.join(args.data_dir, "claim_test.json")
    train_data = read_json(train_data_file)
    test_data = read_json(test_data_file)
    process_train = [t for t in train_data]
    process_test = [d for d in test_data]
    # tokenizer
    tokenizer = AutoTokenizer.from_pretrained(args.pretrained_model_name_or_path)  # AutoTokenizer
    # dataset, dataloader
    train_set = MyDataset(process_train, tokenizer)
    ...
    valid_set = MyDataset(process_test, tokenizer)
    ...
    param_groups = [
        {
            "params": [param for name, param in model.roberta.named_parameters() if param.requires_grad],
            "lr": 3.7e-05,
        },
        {
        ...
        {
            "params": model.claim_classification.parameters(),
            "lr": 5e-05,
        },
        {
            "params": model.span_classifier.parameters(),
    ...
    for epoch in range(args.epochs):
        ...
        results = {
            ...
            'train_classification_f1': train_result['classification_f1'],
            'train_span_accuracy': train_result['span_accuracy'],
            ...
            'dev_classification_f1': dev_result['classification_f1'],
            'dev_span_accuracy': dev_result['span_accuracy'],
        ...
        if dev_result['weighted_f1'] > best_weighted_f1:
            torch.save(model.state_dict(), os.path.join(args.save_weights_path, '{}-{}-epoch{}-macro_f1{:.3f}.pth'.format(
                       args.pretrained_model_name_or_path.split('/')[-1], current_time, epoch, dev_result['weighted_f1'])))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--span_classes', type=int, default=params.span_classes)
