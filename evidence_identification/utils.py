...
def train_one_epoch(model, device, data_loader, epoch, optimizer, lr_scheduler):
    ...
    for step, data in enumerate(data_loader):
       ...
       # claim_attention_mask = dataset['claim_attention_mask'].to(device)
        ...
        weighted_recall = recall_score(ground_truth_labels.tolist(), predicted_labels.tolist(), average='weighted')
        weighted_f1 = f1_score(ground_truth_labels.tolist(), predicted_labels.tolist(), average='weighted')
        weighted_prec = precision_score(ground_truth_labels.tolist(), predicted_labels.tolist(), average='weighted')
        loss.backward()
